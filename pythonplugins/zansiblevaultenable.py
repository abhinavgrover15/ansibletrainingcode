...
def addtovault():
   try:
      FILEPATH = raw_input("Please enter file or directory path(e.g. /etc/ansible/template/times)::")
      os.stat(FILEPATH)
   except:
...
def rekeyfiles():
   try:
      FILEPATH = raw_input("Please enter file or directory path(e.g. /etc/ansible/template/times)::")
      os.stat(FILEPATH)
      VAULTPASS_NEWFILE = raw_input("Please enter new vault password file path(e.g. /tmp/.times_pass)::")
